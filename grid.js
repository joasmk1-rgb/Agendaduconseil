...
// Réduites en phase 9 pour faire tenir plus d'heures à l'écran sans scroller
// (Joas : la grille tenait pas assez large sur son ordi).
export const LAYOUT = {
  timeColWidth: 70,
  dayColWidth: 86,
  monthRowHeight: 20,
...
// Un événement sans champ "status" (tous les événements créés avant cette
// phase) est traité comme "confirme" — comportement inchangé par défaut,
// aucune migration de données nécessaire.
export const EVENT_STATUSES = ["proposition", "a_confirmer", "confirme", "realise", "annule"];
...
export function eventStatus(ev) {
  return ev.status || "confirme";
}
// Un événement encore incertain (proposition / à confirmer) ne doit jamais
// se confondre visuellement avec un événement confirmé — classe CSS dédiée,
// appliquée partout où un événement est affiché (calendrier public + admin).
export function isUncertainEvent(ev) {
  const s = eventStatus(ev);
  return s === "proposition" || s === "a_confirmer";
}
export function eventStatusIcon(ev) {
  const s = eventStatus(ev);
  if (s === "confirme") return ""; // pas de préfixe pour le cas normal, le plus fréquent
  return (EVENT_STATUS_LABELS[s] || s).split(" ")[0] + " ";
}
...
export function renderGridHeaders(container, dates, events) {
  ...
  dates.forEach((date, i) => {
    ...
    const anyUncertain = dayEvents.some(isUncertainEvent);
    const el = document.createElement("div");
    el.className = "cell events-band" + (dayEvents.length ? " has-events" : "") + (anyUncertain ? " has-uncertain-event" : "");
    el.style.gridRow = "3";
    el.style.gridColumn = String(i + 2);
    el.dataset.dateIso = dateISO;
    if (dayEvents.length) {
      el.textContent = dayEvents.map((e) => eventStatusIcon(e) + e.label).join(" · ");
      el.title = dayEvents.map((e) => `${EVENT_STATUS_LABELS[eventStatus(e)]} — ${e.label}`).join("\n") + "\n(clique pour aller marquer ta dispo ici)";
    }
    container.appendChild(el);
  });
}
...
export function renderHourRows(container, dates, times, events, blockedSlots, cellFactory) {
  times.forEach((timeLabel, r) => {
    ...
    dates.forEach((date, i) => {
      ...
      const cellHasUncertainEvent = cellEvents.some(isUncertainEvent);
      ...
      cell.className =
        ...
        (cellHasUncertainEvent ? " has-uncertain-event" : "");
      ...
      if (cellEvents.length) {
        ...
        cell.title = existingTitle + cellEvents.map((e) => `${EVENT_STATUS_LABELS[eventStatus(e)]} — ${e.label}`).join("\n");
      }
      if (cellFactory) {

...
export function isBlocked(date, timeLabel) {
  const dow = date.getDay();
  const minutes = timeToMinutes(timeLabel);
  return CONFIG.blockedSlots.find((b) => {
...
export function renderHourRows(container, dates, times, events, cellFactory) {
  times.forEach((timeLabel, r) => {
    ...
    labelEl.className = "cell time-label";
    ...
    dates.forEach((date, i) => {
      const dateISO = toISODate(date);
      const blocked = isBlocked(date, timeLabel);
      const cellEvents = timedEventsAt(events, dateISO, timeLabel);
      const cell = document.createElement("div");
      cell.className = "cell slot" + (blocked ? " blocked" : "") + (cellEvents.length ? " has-event" : "");

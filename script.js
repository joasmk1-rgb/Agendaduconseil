...
const toggleColTasksBtn = document.getElementById("toggle-col-tasks");
...
const columnEls = { bulk: bulkMarkSection, grid: gridScrollEl, tasks: tasksSection, dashboard: memberDashboardSection };
const columnToggleBtns = { bulk: toggleColBulkBtn, grid: toggleColGridBtn, tasks: toggleColTasksBtn, dashboard: toggleColDashboardBtn };
// Le tableau de bord est nouveau : caché par défaut pour ne pas changer la
// mise en page de ceux qui utilisent déjà le site, sauf s'ils l'activent
// eux-mêmes (mémorisé ensuite comme les 3 autres colonnes).
let visibleColumns = { bulk: true, grid: true, tasks: true, dashboard: false };
try {
...
// ===================== FILTRES (urgence / état) =====================
// S'appliquent aux deux vues (liste et tableau) à la fois.
...
function renderTasks() {
  ...
  const visible = state.tasks.filter((t) => {
    ...
    if (!matchesTaskFilters(t)) return false;
  ...
  visible
    ...
    .forEach((task) => {
      taskListPublic.appendChild(buildTaskListItem(task));
    });
}
...
function renderTaskBoard() {
  ...
  taskBoard.innerHTML = buildTaskBoardColumnsHTML();
}

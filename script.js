...
// venir), on fait défiler la grille jusqu'à cette date à l'ouverture — une
// seule fois par valeur de focusDate, pour ne pas re-forcer le défilement à
// chaque re-rendu si la personne a déjà scrollé ailleurs entre-temps.
let lastAppliedFocusDate = null;
function scrollToFocusDate() {
  const focusDate = state.config && state.config.focusDate;
  if (!focusDate) return;
  const headerCell = gridEl.querySelector(`.cell.day-header[data-date-iso="${focusDate}"]`);
  if (!headerCell) return;
  const targetLeft = headerCell.offsetLeft - Grid.LAYOUT.timeColWidth - 12;
  gridScrollEl.scrollLeft = Math.max(0, targetLeft);
}
...
function renderGrid() {
  ...
  if (state.config.focusDate && state.config.focusDate !== lastAppliedFocusDate) {
    lastAppliedFocusDate = state.config.focusDate;
    scrollToFocusDate();
  }
}

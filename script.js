...
function renderAgendaTab() {
  ...
  items.forEach((item) => {
    ...
    const li = document.createElement("li");
    li.innerHTML = `<span>${item.done ? "✅" : "🟡"} <strong>${item.label}</strong></span><span class="hint" style="padding:0">💬 ${count} — clique pour voir/commenter</span>`;
    li.style.cursor = "pointer";
    li.addEventListener("click", () => openAgendaModal(item.id));
    agendaItemListEl.appendChild(li);
  });
}

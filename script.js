...
const toggleColDashboardBtn = document.getElementById("toggle-col-dashboard");
const memberDashboardSection = document.getElementById("member-dashboard-section");
...
// La colonne "Tâches" a été retirée (remplacée par le tableau de bord, qui
// couvre à la fois "mes tâches" et la vue d'ensemble de l'équipe) — il ne
// reste que 3 colonnes à afficher/masquer.
const columnEls = { bulk: bulkMarkSection, grid: gridScrollEl, dashboard: memberDashboardSection };
const columnToggleBtns = { bulk: toggleColBulkBtn, grid: toggleColGridBtn, dashboard: toggleColDashboardBtn };
let visibleColumns = { bulk: true, grid: true, dashboard: true };
try {
...
function renderMemberDashboard() {
  if (!state.password || visibleColumns.dashboard === false) {
...
// La barre d'onglets entière (pas juste le bouton ODJ) ne sert à rien tant
// que l'admin n'a pas activé l'onglet "Ordre du jour" — dans ce cas, un seul
// bouton "Calendrier" cliquable n'apporterait rien.
function applyAgendaTabVisibility() {
  const enabled = !!(state.config && state.config.agendaTabEnabled);
  publicTabs.classList.toggle("hidden", !enabled);
  agendaTabBtn.classList.toggle("hidden", !enabled);
  if (!enabled && state.activeTab === "agenda") switchTab("calendar");
}
...
function renderAgendaTab() {
  ...
  console.log("[ODJ debug]", {
    meetingId: meeting.id,
    totalAgendaItemsInState: (state.agendaItems || []).length,
    allItemsForThisMeeting: allItems.length,
    topItems: topItems.length,
    sample: allItems.slice(0, 5).map((it) => ({ id: it.id, label: (it.label || "").slice(0, 30), parentId: it.parentId, meetingId: it.meetingId })),
  });

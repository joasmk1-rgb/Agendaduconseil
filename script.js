...
// ===================== TÂCHES =====================
// Visible seulement connecté. On ne montre que les tâches libres (n'importe
// qui peut les prendre) et celles qu'on a soi-même prises (avec un bouton
// pour les lâcher) — celles prises par quelqu'un d'autre disparaissent.
function renderTasks() {
  ...
  const visible = state.tasks.filter((t) => !t.assignedTo || t.assignedTo === state.password);
  if (!visible.length) {
    tasksSection.classList.add("hidden");
    taskListPublic.innerHTML = "";
    return;
  ...
  visible
    ...
    .forEach((task) => {
      ...
      btn.type = "button";
      btn.className = "btn btn-sm " + (task.assignedTo ? "btn-ghost" : "btn-primary");
      btn.textContent = task.assignedTo ? "Abandonner" : "Je m'en occupe";
      btn.addEventListener("click", async () => {
        btn.disabled = true;
        try {
          if (task.assignedTo) {
            await db.unclaimTask(task.id);
          } else {

...
// ---------------------- Tâches ----------------------
// Créées à la main dans l'admin (typiquement à partir d'un PV). Un membre
// connecté peut se les attribuer ("claim") ; une fois prise, elle n'est plus
// proposée aux autres. { label, description?, deadline?, assignedTo?,
// assignedName? } — assignedTo = mot de passe du membre (même identifiant
...
export async function addTask(task) {
  ...
  await setDoc(doc(firestore, "tasks", id), { ...task, createdAt: serverTimestamp() });
}
...
// Transaction : échoue proprement si la tâche vient d'être prise par
// quelqu'un d'autre entre le moment où on l'affiche et le clic (évite que
// deux personnes se l'attribuent en même temps).
export async function claimTask(id, password, name) {
  ...
  await runTransaction(firestore, async (tx) => {
    ...
    if (snap.data().assignedTo) {
      throw new Error("Cette tâche vient d'être prise par quelqu'un d'autre.");
    }
    tx.update(ref, { assignedTo: password, assignedName: name });
  });
}

export async function unclaimTask(id) {
  await updateDoc(doc(firestore, "tasks", id), { assignedTo: deleteField(), assignedName: deleteField() });
}

...
//   polls/{id}                     -> un sondage (voir plus bas)
//   agendaComments/{id}            -> un avis/question sur un point d'ODJ (voir plus bas)
//   agendaProposals/{id}           -> une suggestion de point d'ODJ faite par un conseiller (voir plus bas)
...
export async function addMember(name, password, isAdmin = false, role = "") {
  ...
  // "role" (poste : présidente, vice-présidente, trésorier, secrétaire,
  // responsable communication) : chaîne libre, vide = aucun poste. Aucune
...
// ---------------------- Créneaux "sondés" (façon Doodle intégré) ----------------------
// config/current.pollingSlots = ["YYYY-MM-DD|HH:MM", ...] — créneaux mis en
// avant (orange) sur le calendrier de tout le monde tant qu'un membre n'a pas
// encore répondu dispo/pas dispo dessus. Pas de collection séparée : c'est un
// simple flag sur des créneaux existants de la grille normale.
export async function setPollingSlots(keys) {
  const config = await getConfig();
  const base = config || { rangeDays: 90, includeWeekends: false };
  await saveConfig({ ...base, pollingSlots: keys });
}

export async function clearPollingSlots() {
  await setPollingSlots([]);
}
...
// Remplace toute valeur `null` explicite par deleteField() : permet à
// l'admin.js d'écrire simplement `payload.champ = null` pour effacer un
// champ optionnel lors d'une modification (sinon Firestore refuserait un
// champ présent mais de mauvais type selon firestore.rules).
function withFieldDeletion(patch) {
  const result = {};
...
export async function updateEvent(id, patch) {
  await updateDoc(doc(firestore, "events", id), withFieldDeletion(patch));
}
...
// Changement générique (status, priority, modification complète...).
export async function updateTask(id, patch) {
  await updateDoc(doc(firestore, "tasks", id), withFieldDeletion(patch));
}
...
// Un membre assigné indique avoir terminé sa tâche — ne passe jamais
// directement à "done" : seul l'admin valide réellement (voir dashboard
// "Tâches à valider"). Refuse si la personne n'est pas sur la tâche.
export async function markTaskAwaitingValidation(id, password) {
  const ref = doc(firestore, "tasks", id);
  await runTransaction(firestore, async (tx) => {
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error("Cette tâche n'existe plus.");
    ...
    if (!assignedTo.some((a) => a.password === password)) {
      throw new Error("Tu n'es pas assigné à cette tâche.");
    }
    tx.update(ref, { status: "a_valider" });
  });
}

// ---- Tâches "proposées" (ni libres, ni assignées directement) ----
// proposedTo: [{ password, name }] — personnes ciblées à qui la tâche est
// proposée. Accepter la fait rejoindre assignedTo ; refuser la retire de
// proposedTo ET ajoute son password à declinedBy — sinon, comme la tâche
// reste "libre" en interne (fixedAssignment: false), elle continuait à
// s'afficher comme n'importe quelle tâche disponible à celui qui venait tout
// juste de la refuser. declinedBy ne fait rien disparaître pour les autres :
// ça ne masque cette tâche-là que pour la personne qui a dit non (voir
// script.js, filtre de la colonne "Tâches").
export async function acceptTaskProposal(id, password, name) {
  const ref = doc(firestore, "tasks", id);
  await runTransaction(firestore, async (tx) => {
    const snap = await tx.get(ref);
    if (!snap.exists()) throw new Error("Cette tâche n'existe plus.");
    const data = snap.data();
    const proposed = data.proposedTo || [];
    if (!proposed.some((p) => p.password === password)) return; // pas concerné (déjà traité ailleurs)
    const current = data.assignedTo || [];
    const assignedTo = current.some((a) => a.password === password) ? current : [...current, { password, name }];
    tx.update(ref, {
      assignedTo,
      proposedTo: proposed.filter((p) => p.password !== password),
    });
  });
}

export async function declineTaskProposal(id, password) {
  const ref = doc(firestore, "tasks", id);
  await runTransaction(firestore, async (tx) => {
    const snap = await tx.get(ref);
    if (!snap.exists()) return;
    const data = snap.data();
    const proposed = (data.proposedTo || []).filter((p) => p.password !== password);
    const declinedBy = data.declinedBy || [];
    tx.update(ref, {
      proposedTo: proposed,
      declinedBy: declinedBy.includes(password) ? declinedBy : [...declinedBy, password],
    });
  });
}
...
export async function addProject(project) {
  ...
  await addProjectWithId(id, project);
}

// Variante utilisée par l'import CSV : l'id est généré côté admin.js AVANT
// même la confirmation de l'import (pour que les lignes suivantes du même
// fichier puissent s'y rattacher par nom dès l'aperçu) — on l'écrit donc tel
// quel plutôt que d'en générer un nouveau ici.
export async function addProjectWithId(id, project) {
...
export async function updateDecision(id, patch) {
  await updateDoc(doc(firestore, "decisions", id), withFieldDeletion(patch));
}
...
// ---------------------- ODJ (points d'ordre du jour) ----------------------
// Un point d'ODJ est toujours lié à une réunion (meetingId), avec un champ
// `order` (nombre) qui sert uniquement à définir l'ordre d'affichage — pas de
// vrai drag-and-drop, juste des flèches ↑↓ qui échangent deux `order` voisins.
// { meetingId, label, order, projectId?, decisionId?, done?, parentId? }
// parentId : si présent, ce point est un sous-point de l'agendaItem visé (un
// seul niveau d'imbrication — un sous-point ne peut pas lui-même avoir de
// sous-point, ni côté import CSV ni côté admin).
export function listenAgendaItems(callback) {
  return onSnapshot(collection(firestore, "agendaItems"), (snap) => callback(toDocs(snap)));
}

export async function addAgendaItem(item, forcedId) {
  // forcedId : permet à l'import CSV de fixer l'id à l'avance, pour que le
  // parentId d'un sous-point (calculé avant l'écriture du point principal
  // dans le même lot d'import) pointe bien vers le vrai id du document créé.
  const id = forcedId || `agenda-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`;
  await setDoc(doc(firestore, "agendaItems", id), {
    done: false,
    order: 0,
    ...item,
    createdAt: serverTimestamp(),
...
export async function updateAgendaItem(id, patch) {
  await updateDoc(doc(firestore, "agendaItems", id), patch);
}

export async function removeAgendaItem(id) {
  await deleteDoc(doc(firestore, "agendaItems", id));
}

// ---------------------- Sondages ----------------------
// type: "choice" (une ou plusieurs options prédéfinies, voir "multiple") |
// "yesno" | "text" (réponse libre). status: "open" | "closed".
// responses: [{ password, name, answer }] — answer est soit une string (yesno/text),
// soit un tableau de strings (choice), selon "type". On garde toujours le nom
// du membre à côté de sa réponse : ça permet à l'admin de choisir, à
// l'affichage, entre une vue "résumé" (juste les totaux) et une vue "détail"
// (qui a répondu quoi) — l'anonymat est un choix d'affichage, pas de stockage,
// sinon impossible de savoir qui n'a pas encore répondu.
export function listenPolls(callback) {
  return onSnapshot(collection(firestore, "polls"), (snap) => callback(toDocs(snap)));
}

export async function addPoll(poll) {
  const id = `poll-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`;
  await setDoc(doc(firestore, "polls", id), {
    status: "open",
    multiple: false,
    responses: [],
    ...poll,
...
export async function closePoll(id) {
  await updateDoc(doc(firestore, "polls", id), { status: "closed" });
}

export async function reopenPoll(id) {
  await updateDoc(doc(firestore, "polls", id), { status: "open" });
}
...
// Transaction pour éviter qu'un double-clic (ou deux onglets) n'ajoute deux
// réponses du même membre.
export async function submitPollResponse(id, password, name, answer) {
  const ref = doc(firestore, "polls", id);
  await runTransaction(firestore, async (tx) => {
    const snap = await tx.get(ref);
...
// ---------------------- Commentaires sur un point d'ODJ ----------------------
// Un avis/question/remarque d'un conseiller sur un point précis de l'ordre du
// jour, affiché dans la page publique (onglet "Ordre du jour", si activé par
// l'admin dans Paramètres). { agendaItemId, password, name, text }. Toujours
// nominatif en base (comme le reste du site) — voir note dans firestore.rules
// sur l'absence de vraie sécurité.
export function listenAgendaComments(callback) {
  return onSnapshot(collection(firestore, "agendaComments"), (snap) => callback(toDocs(snap)));
}

export async function addAgendaComment(comment) {
  const id = `agcomment-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`;
  await setDoc(doc(firestore, "agendaComments", id), { ...comment, createdAt: serverTimestamp() });
}

// Modification réservée en pratique à son auteur côté interface (script.js
// n'affiche le bouton "Modifier" que si comment.password === state.password)
// — mais comme pour tout le reste du site, ce n'est pas garanti côté règles.
export async function updateAgendaComment(id, patch) {
  await updateDoc(doc(firestore, "agendaComments", id), patch);
}

export async function removeAgendaComment(id) {
  await deleteDoc(doc(firestore, "agendaComments", id));
}

// ---------------------- Suggestions de point d'ODJ ----------------------
// Un conseiller propose un nouveau point ; reste "pending" tant que l'admin
// ne l'a pas transformé en vrai agendaItem ("accepted") ou rejeté ("rejected").
// { label, text, password, name, status }
export function listenAgendaProposals(callback) {
  return onSnapshot(collection(firestore, "agendaProposals"), (snap) => callback(toDocs(snap)));
}

export async function addAgendaProposal(proposal) {
  const id = `agprop-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`;
  await setDoc(doc(firestore, "agendaProposals", id), { status: "pending", ...proposal, createdAt: serverTimestamp() });
}

export async function updateAgendaProposal(id, patch) {
  await updateDoc(doc(firestore, "agendaProposals", id), patch);
}

export async function removeAgendaProposal(id) {
  await deleteDoc(doc(firestore, "agendaProposals", id));
}

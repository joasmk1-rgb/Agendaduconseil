...
function runAdmin() {
  ...
  eventForm.addEventListener("submit", async (e) => {
    ...
    if (eventProjectSelect.value) payload.projectId = eventProjectSelect.value;
    ...
    currentMeetings
      ...
      .forEach((meeting) => {
        ...
        statusBtn.addEventListener("click", async () => {
          await db.updateMeeting(meeting.id, { status: meeting.status === "done" ? "planned" : "done" });
        });
  ...
  // ---------- Décisions ----------
  const DECISION_STATUS_LABELS = { pending: "❔ En attente", adopted: "✅ Adoptée", rejected: "❌ Rejetée" };
  ...
  decisionForm.addEventListener("submit", async (e) => {
    ...
    if (decisionDateInput.value) payload.date = decisionDateInput.value;
    if (decisionMeetingSelect.value) payload.meetingId = decisionMeetingSelect.value;
    if (decisionProjectSelect.value) payload.projectId = decisionProjectSelect.value;
  ...
  taskSubmitBtn.addEventListener("click", async () => {
    ...
    if (taskDeadlineTimeInput.value) payload.deadlineTime = taskDeadlineTimeInput.value;
    if (taskProjectSelect.value) payload.projectId = taskProjectSelect.value;
    if (taskDecisionSelect.value) payload.decisionId = taskDecisionSelect.value;
  ...
  const TASK_STATUS_LABELS = { todo: "🟡 À faire", in_progress: "🔵 En cours", done: "🟢 Terminé", blocked: "🔴 Bloqué" };

...
// "Admin" (accès complet), OU d'un membre dont le "poste" donne un accès
// partiel ("sous-admin", voir ROLE_ADMIN_VIEWS ci-dessous). Le mot de passe
// fixe ADMIN_PASSPHRASE ne sert qu'à débloquer le tout premier accès complet,
// avant qu'aucun membre ne soit encore admin ; dès qu'un membre est coché
// Admin, ce mot de passe fixe ne fonctionne plus jamais.
//
// Sous-admin par poste : chaque poste ne voit QUE les vues listées ici (plus
// "dashboard", toujours inclus) — pas les vues Membres/Paramètres/Export tant
// que ce n'est pas explicitement listé. Purement une restriction d'affichage
// (voir applyAdminPermissions) : comme le reste du site, ce n'est pas une
...
async function checkAccess() {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    const entered = prompt("Ton mot de passe (si tu es administrateur, ou si tu as un poste donnant accès à certaines sections) :");
    ...
    if (trimmed === ADMIN_PASSPHRASE && !(await db.hasAnyAdmin())) {
      const access = { isAdmin: true, role: "" };
      sessionStorage.setItem("agenda-admin-auth", "ok");
      sessionStorage.setItem("agenda-admin-access", JSON.stringify(access));
      alert(
...
// Masque les entrées de menu (et vues) que ce poste ne doit pas voir. Un
...
function runAdmin() {
  ...
  const eventAvailRequestInput = document.getElementById("event-availrequest-input");
  ...
  const taskModeProposeeBtn = document.getElementById("task-mode-proposee");
  ...
  const taskProposeOptions = document.getElementById("task-propose-options");
  ...
  const taskProposeCheckboxes = document.getElementById("task-propose-checkboxes");
  const taskSubmitBtn = document.getElementById("task-submit-btn");
  const taskCancelEditBtn = document.getElementById("task-cancel-edit-btn");
  const taskList = document.getElementById("task-list");
  const dashboardValidationList = document.getElementById("dashboard-validation-list");
  ...
  const pollTypeChoiceBtn = document.getElementById("poll-type-choice");
  ...
  const pollSlotDate = document.getElementById("poll-slot-date");
  const pollSlotStart = document.getElementById("poll-slot-start");
  const pollSlotEnd = document.getElementById("poll-slot-end");
  ...
  // ajoute une ligne formatée par créneau candidat (jour + date + heures)
  // dans la zone d'options du sondage, sans devoir taper le texte à la main.
  // Ne fait qu'ajouter du texte à la textarea — le sondage lui-même reste un
  // sondage "choix" tout à fait normal, aucune donnée/logique nouvelle.
  pollSlotAddBtn.addEventListener("click", () => {
    if (!pollSlotDate.value || !pollSlotStart.value || !pollSlotEnd.value) {
      alert("Choisis une date, une heure de début et une heure de fin.");
      return;
    }
    const [y, m, d] = pollSlotDate.value.split("-");
    const dow = new Date(`${pollSlotDate.value}T12:00:00`).getDay();
    const label = `${Grid.WEEKDAYS_FULL[dow]} ${d}/${m} — ${pollSlotStart.value.replace(":", "h")}-${pollSlotEnd.value.replace(":", "h")}`;
    const existing = pollOptionsInput.value.split("\n").filter((l) => l.trim() !== "");
    existing.push(label);
    pollOptionsInput.value = existing.join("\n");
  });
  ...
  const responseRangeEnd = document.getElementById("response-range-end");
  const responseRangeClear = document.getElementById("response-range-clear");
  const responseRangeResult = document.getElementById("response-range-result");
  ...
  const taskEventSelect = document.getElementById("task-event-select");
  ...
  const bulkFillDayCheckboxes = document.getElementById("bulk-fill-day-checkboxes");
  const bulkFillStartTime = document.getElementById("bulk-fill-start-time");
  const bulkFillEndTime = document.getElementById("bulk-fill-end-time");
  const bulkFillStartDate = document.getElementById("bulk-fill-start-date");
  const bulkFillEndDate = document.getElementById("bulk-fill-end-date");
  ...
  configForm.addEventListener("submit", async (e) => {
    ...
    try {
      // On repart de currentConfig (pas juste des deux champs de ce
      // formulaire) : saveConfig() remplace tout le document, donc oublier de
      // reprendre les champs existants (blockedSlots, focusDate,
      // agendaTabEnabled...) les effacerait silencieusement à chaque
      // enregistrement des réglages de la fenêtre glissante.
      await db.saveConfig({
        ...currentConfig,
  ...
  memberForm.addEventListener("submit", async (e) => {
    ...
    await db.addMember(name, password, memberAdminInput.checked, memberRoleInput.value);
    ...
    currentMembers
      ...
      .forEach((m) => {
        ...
        if (memberEditingId === m.id) {
          ...
          roleSelect.innerHTML = '<option value="">Poste : aucun</option>' + Object.entries(ROLE_LABELS)
  ...
  function resetEventForm() {
    ...
    eventAvailRequestInput.checked = false;
  }
  ...
  function startEditEvent(ev) {
    ...
    eventAvailRequestInput.checked = !!ev.availabilityRequested;
  ...
  eventForm.addEventListener("submit", async (e) => {
    ...
    payload.availabilityRequested = eventAvailRequestInput.checked ? true : null; // pas de champ stocké si décoché
  ...
  function renderEvents() {
    ...
    currentEvents
      ...
      .forEach((ev) => {
        ...
        const availRequestBadge = ev.availabilityRequested ? ` <code>🙋 dispo demandée</code>` : "";
        li.innerHTML = `<span><strong>${ev.label}</strong> — ${describeEvent(ev)}${requiredBadge}${statusBadge}${sourceBadge}${availRequestBadge}</span>`;
        const actions = document.createElement("span");
        actions.className = "item-actions";
        ...
        removeBtn.textContent = "Retirer";
        removeBtn.addEventListener("click", async () => {
          if (!confirm(`Retirer l'événement "${ev.label}" (${ev.date}) ? Cette action est irréversible.`)) return;
  ...
  // (événements disponibles pour le sélecteur "Événement lié" du formulaire
  // tâche — permet de rattacher une tâche à un événement du calendrier sans
  // forcément passer par une réunion, ex: "Préparer le stand" lié à
  // "Welcome Village").
  function renderTaskEventSelect() {
    if (!taskEventSelect) return;
    const previous = taskEventSelect.value;
    const sorted = currentEvents.slice().sort((a, b) => (a.date || "").localeCompare(b.date || ""));
    taskEventSelect.innerHTML = '<option value="">Événement lié (optionnel)</option>';
    sorted.forEach((ev) => {
      const opt = document.createElement("option");
      opt.value = ev.id;
      const when = ev.allDay ? ev.date : `${ev.date} ${ev.startTime || ""}`.trim();
      opt.textContent = `${ev.label} — ${when}`;
      taskEventSelect.appendChild(opt);
    });
    if (sorted.some((e) => e.id === previous)) taskEventSelect.value = previous;
  }
  ...
  function renderMeetings() {
    ...
    currentMeetings
      ...
      .forEach((meeting) => {
        ...
        removeBtn.addEventListener("click", async () => {
          const when = meeting.startTime ? `${meeting.date} ${meeting.startTime}` : meeting.date;
          if (!confirm(`Supprimer la réunion du ${when} (${ev ? ev.label : "événement supprimé"}) ? Cette action est irréversible.`)) return;
  ...
  function renderAgendaSection(meeting) {
    ...
    if (items.length > 0) {
      ...
      clearBtn.textContent = "Vider les points ODJ de cette réunion";
      clearBtn.addEventListener("click", async () => {
        if (!confirm(`Supprimer les ${items.length} point(s) d'ODJ (et sous-points) de cette réunion ? Cette action est irréversible.`)) return;
        clearBtn.disabled = true;
        try {
          for (const it of items) {
            await db.removeAgendaItem(it.id);
          }
        } finally {
  ...
  function normalizeType(v) {
    ...
    if (t.startsWith("dossier") || t.startsWith("projet")) return "dossier";
    if (t.startsWith("odj") || t.startsWith("point")) return "odj";
    return null;
  }
  function normalizeProjectStatus(v) {
    const t = (v || "").trim().toLowerCase();
  ...
  function normalizeTaskStatus(v) {
    ...
    if (t.includes("cours")) return "in_progress";
    if (t.includes("valider")) return "a_valider";
    if (t.includes("termin")) return "done";
  ...
  function buildImportRow(row, lineNumber) {
    ...
    if (projectRaw) {
      ...
      // Un dossier créé plus haut dans le MÊME fichier CSV n'existe pas
      // encore dans currentProjects (pas encore écrit en base tant que
      // l'import n'est pas confirmé) — on le cherche donc aussi dans
      // importedProjectsThisBatch, alimenté au fur et à mesure par les
      // lignes "dossier" déjà traitées de cet import.
      const pending = importedProjectsThisBatch.find((pp) => pp.name.toLowerCase() === projectRaw.toLowerCase());
      if (p) {
        projectId = p.id;
        projectName = p.name;
      } else if (pending) {
    ...
    if (type === "dossier") {
      ...
      const payload = { name: label, status: normalizeProjectStatus(row["Statut"]) };
      ...
      if (responsableRaw) {
        ...
        if (m) {
          payload.responsible = m.id;
          responsableName = m.name;
        } else {
          warnings.push(`responsable "${responsableRaw}" introuvable, ignoré`);
        }
      }
      const source = (row["Source"] || row["Origine"] || "").trim();
      if (source) payload.source = source;
      if (currentProjects.some((p) => p.name.toLowerCase() === label.toLowerCase())) {
        warnings.push("dossier potentiellement déjà existant (même nom)");
      }
      // Enregistré tout de suite (avant même la confirmation de l'import) :
      // permet aux lignes suivantes du même fichier de s'y rattacher par nom
      // via la colonne "Dossier" (voir plus haut).
      importedProjectsThisBatch.push({ id, name: label });
      const detailParts = [PROJECT_STATUS_LABELS[payload.status] || payload.status, responsableName ? `resp. ${responsableName}` : "", source ? `📌 ${source}` : ""].filter(Boolean);
      return { ok: true, lineNumber, type: "dossier", icon: "📁", label, detail: detailParts.join(" — "), warnings, commit: () => db.addProjectWithId(id, payload) };
    }
    ...
    // Point d'ODJ (et éventuel sous-point) — toujours rattaché à une réunion
    // (obligatoire ici, contrairement à tache/decision où le lien réunion est
    // facultatif) : sans réunion, impossible de savoir où l'afficher côté
    // public. Un seul niveau d'imbrication : un sous-point ne peut pas
    // lui-même avoir de sous-point (choix explicite de Joas).
    if (type === "odj") {
      if (!meetingId) {
        return {
          ok: false,
          lineNumber,
          reason: meetingSourceRaw
            ? `réunion source "${meetingSourceRaw}" introuvable — un point d'ODJ doit être rattaché à une réunion existante`
            : "un point d'ODJ doit avoir une \"Réunion source\" (obligatoire pour ce type)",
        };
      }
      let parentId;
      let parentLabel = "";
      const parentRaw = (row["Sous-point de"] || "").trim();
      if (parentRaw) {
        // On ne cherche le parent QUE parmi les points déjà créés de la même
        // réunion (existants en base, ou créés plus haut dans ce même
        // fichier) — un sous-point d'une autre réunion n'aurait aucun sens.
        const existing = currentAgendaItems.find(
          (it) => it.meetingId === meetingId && !it.parentId && it.label.toLowerCase() === parentRaw.toLowerCase()
        );
        const pending = importedAgendaItemsThisBatch.find(
          (it) => it.meetingId === meetingId && !it.parentId && it.label.toLowerCase() === parentRaw.toLowerCase()
        );
        if (existing) {
          parentId = existing.id;
          parentLabel = existing.label;
        } else if (pending) {
          parentId = pending.id;
          parentLabel = pending.label;
        } else {
          warnings.push(`point parent "${parentRaw}" introuvable pour cette réunion — créé comme point principal à la place`);
        }
      }
      const id = `agenda-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`;
      // Ordre : à la suite du dernier point déjà connu de cette réunion
      // (existants + déjà ajoutés plus haut dans ce même fichier), tous
      // niveaux confondus — l'affichage regroupe ensuite par parentId.
      const existingOfMeeting = currentAgendaItems.filter((it) => it.meetingId === meetingId);
      const pendingOfMeeting = importedAgendaItemsThisBatch.filter((it) => it.meetingId === meetingId);
      const maxOrder = [...existingOfMeeting, ...pendingOfMeeting].reduce((max, it) => Math.max(max, it.order || 0), 0);
      const payload = { meetingId, label, order: maxOrder + 1 };
      if (parentId) payload.parentId = parentId;
      importedAgendaItemsThisBatch.push({ id, label, meetingId, order: payload.order, parentId: parentId || null });
      if (
        currentAgendaItems.some(
          (it) => it.meetingId === meetingId && it.label.toLowerCase() === label.toLowerCase()
        )
      ) {
        warnings.push("point d'ODJ potentiellement déjà existant pour cette réunion (même texte)");
      }
      const detailParts = [meetingLabel ? `🗓️ ${meetingLabel}` : "", parentLabel ? `↳ sous-point de "${parentLabel}"` : ""].filter(Boolean);
      return { ok: true, lineNumber, type: "odj", icon: "📝", label, detail: detailParts.join(" — "), warnings, commit: () => db.addAgendaItem(payload, id) };
    }
  ...
  // Dossiers créés par une ligne "dossier" plus haut dans le fichier CSV en
  // cours de prévisualisation — réinitialisé à chaque nouvel aperçu (voir
  // importPreviewBtn ci-dessous), pour que les lignes suivantes du même
  // fichier puissent s'y rattacher par nom (voir buildImportRow).
  let importedProjectsThisBatch = [];
  // Points d'ODJ créés par une ligne "odj" plus haut dans le fichier en cours
  // de prévisualisation — sert à résoudre la colonne "Sous-point de" quand le
  // point parent est créé dans le MÊME fichier (même mécanique que les
  // dossiers, voir importedProjectsThisBatch). Réinitialisé à chaque aperçu.
  let importedAgendaItemsThisBatch = [];
  ...
  function renderImportPreview() {
    ...
    const typeLabels = { tache: "tâche(s)", evenement: "événement(s)", decision: "décision(s)", dossier: "dossier(s)", odj: "point(s) d'ODJ" };
  ...
  importPreviewBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      const rejected = [];
      pendingImportRows = [];
      importedProjectsThisBatch = [];
      importedAgendaItemsThisBatch = [];
  ...
  function renderDashboard() {
    ...
    dashboardContent.innerHTML = [
      ...
      tile(
        ...
        todoTasks.length,
        (() => {
          // "Non terminées" mélangeait tout (pas commencé, en cours, bloqué,
          // à valider) sous le même chiffre — ça donnait l'impression que
          // rien n'avait avancé alors que ce n'est pas le cas. Détail par
          // statut pour que le chiffre du haut ne fasse plus peur pour rien.
          const byStatus = { todo: 0, in_progress: 0, blocked: 0, a_valider: 0 };
          todoTasks.forEach((t) => {
  ...
  // ---------- Tâches potentiellement terminées, à valider par l'admin ----------
  // Un conseiller a cliqué "J'ai terminé" (status passé à "a_valider" par
  // db.markTaskAwaitingValidation) — ça ne compte JAMAIS comme "Terminée"
  // tant que l'admin n'a pas cliqué "Valider" ici.
  function renderValidationQueue() {
    if (!dashboardValidationList) return;
    const pending = currentTasks.filter((t) => t.status === "a_valider");
    dashboardValidationList.innerHTML = "";
    if (!pending.length) {
      dashboardValidationList.innerHTML = '<li class="hint">Aucune tâche en attente de validation.</li>';
      return;
    }
    pending
      .slice()
      ...
      .forEach((task) => {
        const li = document.createElement("li");
        const names = (task.assignedTo || []).map((a) => a.name).join(", ") || "?";
        li.innerHTML = `<span>${task.label} — indiquée terminée par : ${names}</span>`;
        ...
        rejectBtn.textContent = "↩️ Renvoyer en cours";
        ...
        li.appendChild(actions);
        dashboardValidationList.appendChild(li);
      });
  }
  ...
  function renderProjects() {
    ...
    currentProjects
      ...
      .forEach((p) => {
        ...
        removeBtn.textContent = "Supprimer";
        removeBtn.addEventListener("click", async () => {
          if (!confirm(`Supprimer le dossier "${p.name}" ? Cette action est irréversible (les réunions/tâches/événements/décisions déjà liés ne sont pas supprimés, juste déliés).`)) return;
  ...
  // Une décision qui passe à "Adoptée" (à la création ou via modification)
  // génère automatiquement une tâche de suivi "Mettre en œuvre : ...", liée
  // à cette décision — même principe que les tâches ODJ/PV auto-créées pour
  // les réunions. Jamais deux fois pour la même décision (le libellé sert de
  // repère), et la tâche reste ensuite une tâche normale (modifiable/
  // supprimable/réassignable librement).
  async function createDecisionFollowupTask(decision) {
    const alreadyExists = currentTasks.some((t) => t.decisionId === decision.id && t.label.startsWith("▶️ Mettre en œuvre"));
    if (alreadyExists) return;
    const payload = { label: `▶️ Mettre en œuvre : ${decision.label}`, priority: "normal", decisionId: decision.id };
    const assignee = defaultAutoAssignee();
    if (assignee) {
      payload.fixedAssignment = true;
      payload.assignedTo = [assignee];
  ...
  decisionForm.addEventListener("submit", async (e) => {
    ...
    if (!label) return;
    const previousStatus = editingDecisionId ? ((currentDecisions.find((d) => d.id === editingDecisionId) || {}).status || "pending") : null;
    const newStatus = decisionStatusInput.value;
    const payload = { label, status: newStatus };
    ...
    let decisionId = editingDecisionId;
    if (editingDecisionId) {
      await db.updateDecision(editingDecisionId, payload);
    } else {
      decisionId = await db.addDecision(payload);
    }

    if (newStatus === "adopted" && previousStatus !== "adopted") {
      await createDecisionFollowupTask({ id: decisionId, label });
    }
    resetDecisionForm();
  });
  ...
  function renderDecisions() {
    ...
    currentDecisions
      ...
      .forEach((d) => {
        ...
        removeBtn.textContent = "Supprimer";
        removeBtn.addEventListener("click", async () => {
          if (!confirm(`Supprimer la décision "${d.label}" ? Cette action est irréversible.`)) return;
  ...
  function renderTaskAssigneeCheckboxes() {
    renderCheckboxList(taskAssigneeCheckboxes);
    renderCheckboxList(taskProposeCheckboxes);
  }
  ...
  function resetTaskForm() {
    ...
    if (taskEventSelect) taskEventSelect.value = "";
    ...
    taskProposeCheckboxes.querySelectorAll("input:checked").forEach((c) => (c.checked = false));
  }
  ...
  function startEditTask(task) {
    ...
    if (taskEventSelect) taskEventSelect.value = task.eventId || "";
    ...
    } else if ((task.proposedTo || []).length) {
      setTaskMode("proposee");
      const proposedPasswords = new Set((task.proposedTo || []).map((p) => p.password));
      taskProposeCheckboxes.querySelectorAll("input").forEach((c) => {
        c.checked = proposedPasswords.has(c.value);
      });
    } else {
      setTaskMode("libre");
  ...
  taskSubmitBtn.addEventListener("click", async () => {
    ...
    if (taskEventSelect && (taskEventSelect.value || editingTaskId)) payload.eventId = clearOrOmit(taskEventSelect.value);
    ...
    if (taskMode === "assignee") {
      ...
      payload.proposedTo = editingTaskId ? null : undefined;
    } else if (taskMode === "proposee") {
      const checked = Array.from(taskProposeCheckboxes.querySelectorAll("input:checked"));
      if (!checked.length) {
        alert("Choisis au moins une personne à qui proposer la tâche.");
        return;
      }
      payload.fixedAssignment = false;
      payload.proposedTo = checked.map((c) => ({ password: c.value, name: c.dataset.name }));
      if (!editingTaskId) payload.assignedTo = [];
      payload.requiredCount = Math.max(1, Number(taskRequiredInput.value) || 1);
      // Si on repropose explicitement à quelqu'un qui avait refusé, on
      // l'enlève de declinedBy pour qu'elle lui réapparaisse (voir
      // acceptTaskProposal/declineTaskProposal côté db.js).
      if (editingTaskId) {
        const existing = currentTasks.find((t) => t.id === editingTaskId);
        const reproposedPasswords = new Set(payload.proposedTo.map((p) => p.password));
        const remainingDeclined = ((existing && existing.declinedBy) || []).filter((p) => !reproposedPasswords.has(p));
        payload.declinedBy = remainingDeclined.length ? remainingDeclined : null;
      }
    } else {
      payload.fixedAssignment = false;
      if (!editingTaskId) payload.assignedTo = [];
  ...
  function matchesTaskFilters(task) {
    ...
    if (priority && (task.priority || "normal") !== priority) return false;
    if (status && (task.status || "todo") !== status) return false;
    return true;
  }
  ...
  function renderTasks() {
    ...
    currentTasks
      ...
      .forEach((task) => {
        ...
        if (task.eventId) {
          const linkedEvent = currentEvents.find((e) => e.id === task.eventId);
          if (linkedEvent) parts.push(`📅 ${linkedEvent.label} (${linkedEvent.date})`);
        }
        ...
        if (proposedTo.length) parts.push(`🎯 proposée à : ${proposedTo.map((p) => p.name).join(", ")} (en attente de réponse)`);
        ...
        removeBtn.addEventListener("click", async () => {
          if (!confirm(`Supprimer la tâche "${task.label}" ? Cette action est irréversible.`)) return;
  ...
  // Un membre "a répondu dans la plage [startISO, endISO]" s'il a au moins un
  // créneau marqué (dispo OU pas dispo — les deux comptent comme "a répondu",
  // seul "rien du tout" compte comme "n'a pas répondu") dont la date tombe
  // dans cette plage (bornes incluses). Comparaison en chaîne "YYYY-MM-DD",
  // valide car c'est déjà le format ISO utilisé partout dans les clés.
  function hasRespondedInRange(marks, startISO, endISO) {
    return Object.keys(marks || {}).some((key) => {
  ...
  function renderResponseCount() {
    const startISO = responseRangeStart.value;
    const endISO = responseRangeEnd.value;
    if (startISO && endISO) {
      const responded = [];
      const notResponded = [];
      currentMembers.forEach((m) => {
        const record = currentAvailability.find((r) => r.id === m.id);
        if (record && hasRespondedInRange(record.marks, startISO, endISO)) responded.push(m.name);
        else notResponded.push(m.name);
      });
      responded.sort((a, b) => a.localeCompare(b));
      notResponded.sort((a, b) => a.localeCompare(b));
      responseCountEl.textContent = String(responded.length);
      memberCountEl.textContent = String(currentMembers.length);
      responseRangeResult.innerHTML = `
        <p class="hint">Compté : au moins une dispo ou indispo entre le ${startISO} et le ${endISO}.</p>
        <p>✅ Ont répondu dans cette plage (${responded.length}) : ${responded.join(", ") || "—"}</p>
        <p>❔ N'ont pas répondu dans cette plage (${notResponded.length}) : ${notResponded.join(", ") || "—"}</p>
      `;
      return;
    }
    responseRangeResult.innerHTML = "";
  ...
  responseRangeStart.addEventListener("change", renderResponseCount);
  responseRangeEnd.addEventListener("change", renderResponseCount);
  responseRangeClear.addEventListener("click", () => {
    responseRangeStart.value = "";
    responseRangeEnd.value = "";
    renderResponseCount();
  });
  ...
  // appliquée à une sélection de membres d'un coup plutôt qu'à soi-même —
  // pratique quand tout un sous-groupe communique les mêmes dispos.
  const BULK_FILL_DAY_ORDER = [1, 2, 3, 4, 5, 6, 0]; // lundi → dimanche

  function renderBulkFillDayCheckboxes() {
    if (bulkFillDayCheckboxes.childElementCount) return; // construit une seule fois
    BULK_FILL_DAY_ORDER.forEach((dow) => {
      const label = document.createElement("label");
      label.className = "checkbox-group";
      const input = document.createElement("input");
      input.type = "checkbox";
      input.className = "bulk-fill-day-checkbox";
      input.value = String(dow);
      input.checked = true;
      input.disabled = true;
      label.appendChild(input);
      label.appendChild(document.createTextNode(" " + Grid.WEEKDAYS_FULL[dow].slice(0, 3)));
      bulkFillDayCheckboxes.appendChild(label);
    });
  }
  bulkFillDayAll.addEventListener("change", () => {
    const allChecked = bulkFillDayAll.checked;
    bulkFillDayCheckboxes.querySelectorAll(".bulk-fill-day-checkbox").forEach((c) => {
      c.disabled = allChecked;
      c.checked = allChecked;
    });
  });
  renderBulkFillDayCheckboxes();

  function renderBulkFillMemberCheckboxes() {
    renderCheckboxList(bulkFillMemberCheckboxes);
  }

  let bulkFillMode = "available";
  function setBulkFillMode(mode) {
    bulkFillMode = mode;
    bulkFillModeAvailableBtn.classList.toggle("active", mode === "available");
    bulkFillModeUnavailableBtn.classList.toggle("active", mode === "unavailable");
    bulkFillModeClearBtn.classList.toggle("active", mode === "clear");
  }
  bulkFillModeAvailableBtn.addEventListener("click", () => setBulkFillMode("available"));
  bulkFillModeUnavailableBtn.addEventListener("click", () => setBulkFillMode("unavailable"));
  bulkFillModeClearBtn.addEventListener("click", () => setBulkFillMode("clear"));

  bulkFillApplyBtn.addEventListener("click", async () => {
    const checkedMembers = Array.from(bulkFillMemberCheckboxes.querySelectorAll("input:checked"));
    if (!checkedMembers.length) {
      bulkFillResult.textContent = "Choisis au moins un membre.";
      return;
    }
    const selectedDays = new Set(
      Array.from(bulkFillDayCheckboxes.querySelectorAll(".bulk-fill-day-checkbox"))
        .filter((c) => bulkFillDayAll.checked || c.checked)
        .map((c) => Number(c.value))
    );
    const startTime = bulkFillStartTime.value || null;
    const endTime = bulkFillEndTime.value || null;
    const startDateISO = bulkFillStartDate.value || null;
    const endDateISO = bulkFillEndDate.value || null;
    ...
    if (!matchKeys.length) {
      bulkFillResult.textContent = "Aucun créneau ne correspond à ces critères.";
      return;
    }
    ...
    try {
      ...
      bulkFillResult.textContent = `${matchKeys.length} créneau(x) mis à jour pour ${checkedMembers.length} membre(s).`;
      if (fillState.password && checkedMembers.some((c) => c.value === fillState.password)) {
        ...
        // d'être modifié : on recharge pour que la grille reflète le nouvel état.
      ...
      console.error(err);
      bulkFillResult.textContent = "Échec de l'enregistrement, réessaie.";
    }
  ...
  // ---------- Sondages ----------
  let pollType = "choice"; // "choice" | "yesno" | "text"
  ...
  pollSubmitBtn.addEventListener("click", async () => {
    ...
    if (pollType === "choice") {
      const options = pollOptionsInput.value
        ...
        .filter(Boolean);
      if (options.length < 2) {
        alert("Il faut au moins 2 options pour un sondage à choix.");
        return;
      }
    ...
    try {
      ...
      console.error(err);
      alert("Échec de la création du sondage : " + (err.message || "erreur inconnue") + " — réessaie.");
    }
    pollSubmitBtn.disabled = false;
  });

  // "résumé" (juste les totaux, sans dire qui a répondu quoi) par défaut ;
  // "détail" (nominatif) en option, par sondage — mémorisé juste en mémoire
  ...
  function summarizePoll(poll) {
    ...
    return { notResponded, options, counts, responses };
  }

  function pollResultsHTML(poll) {
    const summary = summarizePoll(poll);
    const showDetail = pollDetailView.has(poll.id);
    const notRespondedHtml = `<p class="hint">N'ont pas répondu (${summary.notResponded.length}) : ${summary.notResponded.map((m) => m.name).join(", ") || "—"}</p>`;

    if (poll.type === "text") {
      const list = summary.texts.length
        ? summary.texts
            .map((t) => `<li>${showDetail ? `<strong>${t.name}</strong> : ` : ""}${t.answer || "(vide)"}</li>`)
            .join("")
        : "<li>Aucune réponse pour l'instant.</li>";
      return `<ul class="poll-text-answers">${list}</ul>${notRespondedHtml}`;
    }

    const total = (poll.responses || []).length || 1;
    const barsHtml = summary.options
      .map((o) => {
        const n = summary.counts[o] || 0;
        const pct = Math.round((n / total) * 100);
        return `<div class="poll-bar-row"><span class="poll-bar-label">${o}</span><div class="poll-bar-track"><div class="poll-bar-fill" style="width:${pct}%"></div></div><span class="poll-bar-count">${n}</span></div>`;
      })
      .join("");
    let detailHtml = "";
    if (showDetail) {
      detailHtml =
        "<ul class='poll-detail-list'>" +
        (summary.responses.length
          ? summary.responses
              .map((r) => `<li><strong>${r.name}</strong> : ${Array.isArray(r.answer) ? r.answer.join(", ") : r.answer}</li>`)
              .join("")
  ...
  function renderPolls() {
    ...
    currentPolls
      ...
      .forEach((poll) => {
        ...
        toggleDetailBtn.className = "btn btn-ghost btn-sm";
        toggleDetailBtn.textContent = pollDetailView.has(poll.id) ? "Vue résumé" : "Vue détail (nominatif)";
        toggleDetailBtn.addEventListener("click", () => {
          if (pollDetailView.has(poll.id)) pollDetailView.delete(poll.id);
          else pollDetailView.add(poll.id);
          renderPolls();
        });
        ...
        exportBtn.type = "button";
        exportBtn.className = "btn btn-ghost btn-sm";
        exportBtn.textContent = "⬇️ Exporter CSV";
        exportBtn.addEventListener("click", () => exportPollCSV(poll));
        actions.appendChild(exportBtn);
        ...
        deleteBtn.addEventListener("click", async () => {
          if (confirm(`Supprimer le sondage "${poll.question}" et toutes ses réponses ?`)) {
            await db.removePoll(poll.id);
          }
        });
  ...
  // Champ CSV français (Excel FR lit ";" par défaut) — mêmes règles
  // d'échappement que le parseur CSV maison plus haut : on met entre
  // guillemets dès qu'il y a un ";", un retour à la ligne ou un guillemet
  // dans le texte (et on double les guillemets internes).
  ...
  function buildPollCSV(poll) {
    ...
    rows.push(["Type", { choice: "Choix", yesno: "Oui/Non", text: "Libre" }[poll.type] || poll.type]);
    rows.push(["Statut", poll.status === "open" ? "Ouvert" : "Fermé"]);
    rows.push([]);

    if (poll.type === "text") {
      rows.push(["Membre", "Réponse"]);
      summary.texts.forEach((t) => rows.push([t.name, t.answer]));
    } else {
      rows.push(["Option", "Nombre de réponses"]);
      summary.options.forEach((o) => rows.push([o, summary.counts[o] || 0]));
      rows.push([]);
      rows.push(["Détail (qui a répondu quoi)"]);
      rows.push(["Membre", "Réponse"]);
      summary.responses.forEach((r) => rows.push([r.name, Array.isArray(r.answer) ? r.answer.join(", ") : r.answer]));
    }

    rows.push([]);
    rows.push(["N'ont pas répondu"]);
    summary.notResponded.forEach((m) => rows.push([m.name]));

    return rows.map((row) => row.map(csvField).join(";")).join("\r\n");
  }

  function exportPollCSV(poll) {
    const csv = buildPollCSV(poll);
    // ﻿ (BOM) devant : sans ça, Excel en français peut afficher les
    // accents de travers à l'ouverture d'un CSV UTF-8.
    const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const safeName = poll.question.replace(/[^a-z0-9]+/gi, "-").replace(/^-+|-+$/g, "").slice(0, 50) || "sondage";
    a.href = url;
    a.download = `sondage-${safeName}.csv`;
  ...
  // ---------- Export CSV complet (compte-rendu à l'instant t, pensé pour être relu par une IA) ----------
  function memberName(password) {
    const m = currentMembers.find((x) => x.id === password);
    return m ? m.name : password || "";
  }

  function formatPeople(list) {
    return (list || []).map((a) => a.name || memberName(a.password)).join(", ");
  }

  const TASK_STATUS_FR = { todo: "À faire", in_progress: "En cours", a_valider: "À valider", done: "Terminé", blocked: "Bloqué" };
  const TASK_PRIORITY_FR = { urgent: "Urgent", important: "Important", normal: "Normal" };
  const DECISION_STATUS_FR = { pending: "En discussion", adopted: "Adoptée", rejected: "Rejetée" };
  const PROJECT_STATUS_FR = { todo: "À faire", in_progress: "En cours", paused: "En pause", done: "Terminé" };

  function buildFullExportCSV() {
    const rows = [];
    const today = new Date().toISOString().slice(0, 10);
    rows.push([`Export complet — Conseil étudiant — généré le ${today}`]);
    rows.push([]);

    rows.push(["## MEMBRES"]);
    rows.push(["Nom", "Admin"]);
    currentMembers.forEach((m) => rows.push([m.name, m.isAdmin ? "Oui" : "Non"]));
    rows.push([]);

    rows.push(["## DISPONIBILITÉS (résumé — le détail créneau par créneau n'est pas inclus, trop volumineux)"]);
    rows.push(["Membre", "Nombre de créneaux marqués", "Dernière mise à jour"]);
    currentAvailability.forEach((a) => {
      const marks = a.marks || {};
      const count = Object.keys(marks).length;
      const updated = a.updatedAt && a.updatedAt.toDate ? a.updatedAt.toDate().toISOString().slice(0, 10) : "";
      rows.push([a.name || memberName(a.id), count, updated]);
    });
    const respondedIds = new Set(currentAvailability.map((a) => a.id));
    const noResponse = currentMembers.filter((m) => !respondedIds.has(m.id));
    if (noResponse.length) {
      rows.push([]);
      rows.push(["N'ont jamais rempli leurs disponibilités"]);
      noResponse.forEach((m) => rows.push([m.name]));
    }
    rows.push([]);

    rows.push(["## TÂCHES"]);
    rows.push(["Libellé", "Statut", "Priorité", "Échéance", "Mode", "Assigné(e) à", "Proposé à (en attente)", "A refusé", "Description"]);
    currentTasks.forEach((t) => {
      const mode = (t.proposedTo || []).length ? "Proposée" : t.fixedAssignment ? "Assignée directement" : "Libre";
      rows.push([
        t.label || "",
        TASK_STATUS_FR[t.status] || t.status || "",
        TASK_PRIORITY_FR[t.priority] || t.priority || "",
        t.deadline ? `${t.deadline}${t.deadlineTime ? " " + t.deadlineTime : ""}` : "",
        mode,
        formatPeople(t.assignedTo),
        formatPeople(t.proposedTo),
        (t.declinedBy || []).map(memberName).join(", "),
        t.description || "",
      ]);
    });
    rows.push([]);
    ...
    rows.push(["## RÉUNIONS"]);
    rows.push(["Date", "Heure début", "Heure fin", "Lieu", "Statut", "Présents", "Absents", "Procurations"]);
    currentMeetings.forEach((m) => {
      const attendance = m.attendance || {};
      const present = Object.entries(attendance).filter(([, v]) => v === "present").map(([p]) => memberName(p));
      const absent = Object.entries(attendance).filter(([, v]) => v === "absent").map(([p]) => memberName(p));
      const proxies = Object.entries(m.proxies || {}).map(([p, holder]) => `${memberName(p)} → ${memberName(holder)}`);
      rows.push([
        m.date || "",
        m.startTime || "",
        m.endTime || "",
        m.location || "",
        m.status === "done" ? "Terminée" : "Prévue",
        present.join(", "),
        absent.join(", "),
        proxies.join(", "),
      ]);
    ...
    rows.push(["Titre", "Date", "Heure début", "Heure fin", "Statut", "Description"]);
    currentEvents.forEach((e) => {
      rows.push([e.title || e.label || "", e.date || "", e.startTime || "", e.endTime || "", e.status || "", e.description || ""]);
    });
    rows.push([]);

    rows.push(["## DÉCISIONS"]);
    rows.push(["Libellé", "Statut", "Date", "Description"]);
    currentDecisions.forEach((d) => {
      rows.push([d.label || "", DECISION_STATUS_FR[d.status] || d.status || "", d.date || "", d.description || ""]);
    });
    rows.push([]);

    rows.push(["## DOSSIERS"]);
    rows.push(["Nom", "Statut", "Priorité", "Responsable", "Description"]);
    currentProjects.forEach((p) => {
      rows.push([
        p.name || "",
        PROJECT_STATUS_FR[p.status] || p.status || "",
        p.priority || "",
        p.responsible ? memberName(p.responsible) : "",
        p.description || "",
      ]);
    });
    rows.push([]);
    ...
    rows.push(["## SONDAGES"]);
    rows.push(["Question", "Type", "Statut", "Réponses (détail)", "N'ont pas répondu"]);
    currentPolls.forEach((p) => {
      const responded = new Set((p.responses || []).map((r) => r.password));
      const notResponded = currentMembers.filter((m) => !responded.has(m.id)).map((m) => m.name);
      const detail = (p.responses || [])
        .map((r) => `${r.name}: ${Array.isArray(r.answer) ? r.answer.join("+") : r.answer}`)
        .join(" | ");
      rows.push([
        p.question || "",
        { choice: "Choix", yesno: "Oui/Non", text: "Libre" }[p.type] || p.type || "",
  ...
  function exportFullCSV() {
    ...
    // BOM devant : sans ça, Excel en français peut afficher les accents de
    // travers à l'ouverture d'un CSV UTF-8.
    const blob = new Blob(["﻿" + csv], { type: "text/csv;charset=utf-8;" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    const today = new Date().toISOString().slice(0, 10);
    a.href = url;
    a.download = `conseil-export-${today}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    exportResult.textContent = "Export généré et téléchargé.";
  }

  // ---------- Suggestions de point d'ODJ (proposées par les conseillers) ----------
  // "Accepter" crée un vrai point d'ODJ sur la prochaine réunion prévue (même
  // logique que le "tableau de bord" pour identifier "la prochaine réunion" :
  // la première réunion status="planned" dont la date n'est pas passée) et à
  // la suite du dernier point existant (order max + 1). Si aucune réunion
  // n'est prévue, on prévient plutôt que d'écrire un agendaItem orphelin.
  function nextPlannedMeeting() {
    const todayISO = Grid.toISODate(new Date());
  ...
  function renderAgendaProposals() {
    if (!agendaProposalList) return;
    const pending = currentAgendaProposals
      .filter((p) => p.status === "pending")
      .sort((a, b) => (a.createdAt?.toMillis?.() || 0) - (b.createdAt?.toMillis?.() || 0));
    if (!pending.length) {
      agendaProposalList.innerHTML = `<li class="hint">Aucune suggestion en attente.</li>`;
      return;
    }
    agendaProposalList.innerHTML = "";
    pending.forEach((p) => {
      const li = document.createElement("li");
      const span = document.createElement("span");
      span.innerHTML = `💬 <strong>${p.label}</strong>${p.text ? " — " + p.text : ""} <span class="hint" style="padding:0">(par ${p.name})</span>`;
      li.appendChild(span);
      ...
      acceptBtn.addEventListener("click", async () => {
        const meeting = nextPlannedMeeting();
        if (!meeting) {
          alert("Aucune réunion prévue à venir — crée d'abord la prochaine réunion (onglet Réunions) avant d'accepter cette suggestion.");
          return;
        }
        acceptBtn.disabled = true;
        try {
          const itemsOfMeeting = currentAgendaItems.filter((it) => it.meetingId === meeting.id);
          const maxOrder = itemsOfMeeting.reduce((max, it) => Math.max(max, it.order || 0), 0);
          await db.addAgendaItem({ meetingId: meeting.id, label: p.label, order: maxOrder + 1 });
          await db.updateAgendaProposal(p.id, { status: "accepted" });
        } catch (err) {
          console.error(err);
          alert("Échec de la validation — réessaie.");
      ...
      rejectBtn.addEventListener("click", async () => {
        ...
        try {
          await db.updateAgendaProposal(p.id, { status: "rejected" });
        } catch (err) {
          console.error(err);
      ...
      actions.appendChild(rejectBtn);
      li.appendChild(actions);
      agendaProposalList.appendChild(li);
    });
  }
  ...
  db.listenAgendaProposals((proposals) => {
    currentAgendaProposals = proposals;
    renderAgendaProposals();
  });
}

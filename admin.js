...
function runAdmin() {
  ...
  function buildImportRow(row, lineNumber) {
    ...
    if (type === "tache") {
      ...
      const detailParts = [deadline ? `échéance ${deadline}${deadlineTime ? " " + deadlineTime : ""}` : "", responsableName ? `resp. ${responsableName}` : "", projectName ? `📁 ${projectName}` : "", decisionLabelResolved ? `🗳️ ${decisionLabelResolved}` : ""].filter(Boolean);
    ...
    if (type === "decision") {
      ...
      if (projectId) payload.projectId = projectId;
      const detailParts = [date, projectName ? `📁 ${projectName}` : ""].filter(Boolean);

...
function runAdmin() {
  ...
  function renderEvents() {
    ...
    currentEvents
      ...
      .forEach((ev) => {
        ...
        li.innerHTML = `<span><strong>${ev.label}</strong> — ${describeEvent(ev)}${requiredBadge}${statusBadge}</span>`;
        const actions = document.createElement("span");
        actions.className = "item-actions";
  ...
  function normalizeTaskStatus(v) {
    const t = (v || "").trim().toLowerCase();
    if (t.includes("proposition")) return "proposition";
  ...
  function buildImportRow(row, lineNumber) {
    ...
    if (meetingSourceRaw) {
      const matches = currentMeetings.filter((m) => m.date === meetingSourceRaw);
      if (matches.length) {
    ...
    if (type === "tache") {
      ...
      const detailParts = [deadline ? `échéance ${deadline}${deadlineTime ? " " + deadlineTime : ""}` : "", responsableName ? `resp. ${responsableName}` : "", projectName ? `📁 ${projectName}` : "", decisionLabelResolved ? `🗳️ ${decisionLabelResolved}` : "", meetingLabel ? `🗓️ ${meetingLabel}` : ""].filter(Boolean);
    ...
    if (type === "evenement") {
      ...
      if (projectId) payload.projectId = projectId;
      const detailParts = [date + (heure ? " " + heure : ""), projectName ? `📁 ${projectName}` : "", status !== "confirme" ? Grid.EVENT_STATUS_LABELS[status] : ""].filter(Boolean);
    ...
    if (type === "decision") {
      ...
      if (projectId) payload.projectId = projectId;
      if (meetingId) payload.meetingId = meetingId;
      const detailParts = [date, projectName ? `📁 ${projectName}` : "", meetingLabel ? `🗓️ ${meetingLabel}` : ""].filter(Boolean);

...
function runAdmin() {
  ...
  let currentConfig = { rangeDays: 30, includeWeekends: false };
  let currentMembers = [];
  let currentEvents = [];
  ...
  function fillConfigForm(config) {
    rangeSelect.value = String((config && config.rangeDays) || 30);
  ...
  memberForm.addEventListener("submit", async (e) => {
    ...
    await db.addMember(name, password);
  ...
  function renderMembers() {
    ...
    currentMembers
      ...
      .forEach((m) => {
        ...
        li.innerHTML = `<span>${m.name} <code>${m.id}</code></span>`;
